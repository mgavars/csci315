...
int main(int argc, char *argv[]) {
  ...
  if (fstat(fd, &file_info) == 0) {
    ...
    umode = file_info.st_mode >> 6; // TO-DO
    
    // gmode comes from the middle 3 bits in mode
    gmode = file_info.st_mode >> 3 & 0x111; // TO-DO
    
    // omode comes from the low 3 bits in mode
    omode = file_info.st_mode << 6; // TO-DO
    ...
    struct passwd *filler = getpwuid(file_info.st_uid);
    
    //if (getpwuid_r(
    printf(" owner user name = %s\n", filler->pw_name); // TO-DO: man getpwuid 
